...
dwt.out "<html>"& vbCrLf
dwt.out "<head>" & vbCrLf
dwt.out "<title>信息管理系统-月计划备件汇总</title>"& vbCrLf
...
dim rscj,sqlcj
	...
	dwt.out"		<H3 style='MARGIN-BOTTOM: 5px'>添加月计划备件</H3>"& vbCrLf
	dwt.out"		<DIV id=form-ct>"& vbCrLf
	dwt.out "<form method='post' class='x-form' action='yjhbj.asp' name='form1' onsubmit='javascript:return checkadd();'>"
	dwt.out"			<DIV class='x-form-ct'>"& vbCrLf

	Dwt.out"			  <Div class='x-form-item'>"& vbCrLf
	Dwt.out"				<LABEL style='WIDTH: 85px' >备件计划日期:</LABEL>"& vbCrLf
	Dwt.out"				<Div class=x-form-element style='PADDING-LEFT: 80px'>"& vbCrLf
    dwt.out"<input name='jx_nd' style='WIDTH: 175px' onClick='new Calendar(0).show(this)' readOnly >"
	Dwt.out "</select>"
	Dwt.out"				</Div>"& vbCrLf
	Dwt.out"			  </Div>"& vbCrLf
...
sub saveadd()    
	...
	rsadd.close
	set rsadd=nothing
	dwt.out"<Script Language=Javascript>location.href='yjhbj.asp';</Script>"
end sub


sub saveedit()    
	set rsedit=server.createobject("adodb.recordset")
	sqledit="select * from yjh_bj where id="&ReplaceBadChar(Trim(request("ID")))
...
sub del()
	...
	sqldel="delete * from yjh_bj where id="&qxdjid
...
sub edit()
	...
	set rsedit=server.createobject("adodb.recordset")
	sqledit="select * from yjh_bj where id="&id
	...
	dwt.out"	<DIV class=x-box-mr>"& vbCrLf
	dwt.out"	  <DIV class=x-box-mc>"& vbCrLf
	dwt.out"		<H3 style='MARGIN-BOTTOM: 5px'>编辑月计划备件</H3>"& vbCrLf
	dwt.out"		<DIV id=form-ct>"& vbCrLf
    dwt.out"<form method='post' action='yjhbj.asp' name='form1' onsubmit='javascript:return checkadd();'>"
	dwt.out"			<DIV class='x-form-ct'>"& vbCrLf

	Dwt.out"			  <Div class='x-form-item'>"& vbCrLf
	Dwt.out"				<LABEL style='WIDTH: 85px' >备件计划日期:</LABEL>"& vbCrLf
	Dwt.out"				<Div class=x-form-element style='PADDING-LEFT: 80px'>"& vbCrLf
    dwt.out"<input name='jx_nd' style='WIDTH: 175px' onClick='new Calendar(0).show(this)' readOnly value='"&rsedit("jx_nd")&"' >"
	Dwt.out"				</Div>"& vbCrLf
	Dwt.out"			  </Div>"& vbCrLf
	Dwt.out"			  <Div class=x-form-clear-left></Div>"& vbCrLf
...
sub main()
	...
	sqlqxdj="SELECT * from yjh_bj  where 1=1"
	...
	dwt.out "        <SPAN class='x-layout-panel-hd-text' style:'top:0px;'>月计划备件汇总"&title&"</span>"& vbCrLf
	dwt.out "     </div>"& vbCrLf
	...
	if rsqxdj.eof and rsqxdj.bof then 
	   message "未找到相关内容"
		...
		dwt.out "      <td class='x-td'><DIV class='x-grid-hd-text'>备件计划日期</div></td>"
	   ...
	   do while not rsqxdj.eof and rowcount>0
			...
			'call editdel(rsqxdj("id"),rsqxdj("sscj"),"yjhbj.asp?action=edit&id=","yjhbj.asp?action=del&id=")
            if session("groupid")=7 then
 
    dwt.out "<a href=yjhbj.asp?action=edit&id="&rsqxdj("id")&">编辑</a>&nbsp;"
    dwt.out "<a href=yjhbj.asp?action=del&id="&rsqxdj("id")&" onClick=""return confirm('确定要删除此记录吗？');"">删除</a>"
 end if 
 dwt.out "&nbsp;"
...
sub search()
	...
	Dwt.out "<form method='Get' name='SearchForm' action='yjhbj.asp'>" & vbCrLf
	
	if displaypagelevelh(session("groupid"),1,session("pagelevelid")) then Dwt.out "<a href=""yjhbj.asp?action=add"">添加记录</a>&nbsp;&nbsp;"
	...
	sqlgh="SELECT distinct sscj from yjh_bj"
    ...
    do while not rsgh.eof
		cjid=cint(rsgh("sscj"))
		sql="SELECT count(jx_id) FROM yjh_bj WHERE sscj="&cjid
		if request("jx_nd")<>"" then sql=sql&" and jx_nd="&request("jx_nd")
		jx_numb=connnd.Execute(sql)(0)
        
		if jx_numb<>0 then 
			'i=i+1
			Dwt.out"<option  value='yjhbj.asp?sscj="&cjid
