...
dwt.out  "<html>"& vbCrLf
dwt.out  "<head>" & vbCrLf
dwt.out  "<title>信息管理系统车辆信息管理页</title>"& vbCrLf
...
sub add()
   dwt.out "<br><form method='post' action='jlclgl.asp' name='form1'>"
   ...
   dwt.out "<div align='center'><strong>添加车辆管理信息表</strong></div></td>    </tr>"
   ...
   dwt.out "<IMG src='/images2006/calendar/date_selector.gif' border='0' align='absMiddle'></a></td></tr>"& vbCrLf
	...
	dwt.out "<input name='action' type='hidden' id='action' value='saveadd'>    <input  type='submit' name='Submit' value=' 添 加 ' style='cursor:hand;'>&nbsp;<input name='Cancel' type='button' id='Cancel' value=' 取 消 ' onClick=""window.location.href='jlclgl.asp';"" style='cursor:hand;'></td>  </tr>"
	dwt.out "</table></form>"
end sub	

sub saveadd()    
	  '保存新增用户
      if Trim(Request("zbb_title"))="" then
	  response.write("<Script language=JavaScript>alert('请添加标题!');history.back();</Script>")
      else
      set rsadd=server.createobject("adodb.recordset")
      sqladd="select * from jlclgl" 
      rsadd.open sqladd,connj,1,3
      rsadd.addnew
	  ...
	  dwt.out "<Script Language=Javascript>location.href='jlclgl.asp';</Script>"
end sub
...
sub edit()
   ...
   sqledit="select * from jlclgl where id="&id
   rsedit.open sqledit,connj,1,1

   dwt.out "<br><form method='post' action='jlclgl.asp' name='form1'>"
   dwt.out "<table width='90%' border='0' align='center' cellpadding='2' cellspacing='1' class='border' >"
   dwt.out "<tr class='title'><td height='22' colspan='2'>"
   dwt.out "<div align='center'><strong>编辑车辆信息</strong></div></td>    </tr>"
   dwt.out "<tr class='tdbg'><td width='12%' align='right' class='tdbg'>"
   dwt.out "<strong>标&nbsp;&nbsp;题：</strong></td>"
   ...
   dwt.out "<IMG src='/images2006/calendar/date_selector.gif' border='0' align='absMiddle'></a></td></tr>"& vbCrLf
    ...
    dwt.out "<tr> <td height='40' colspan='2' align='center' class='tdbg'>"
	dwt.out "<input name='action' type='hidden' id='action' value='saveedit'>	<input type='hidden' name='id' value='"&id&"'>   <input  type='submit' name='Submit' value=' 保 存 ' style='cursor:hand;'>&nbsp;<input name='Cancel' type='button' id='Cancel' value=' 取 消 ' onClick=""window.location.href='jlclgl.asp';"" style='cursor:hand;'></td>  </tr>"
	dwt.out "</table></form>"
...
'编辑保存
set rsedit=server.createobject("adodb.recordset")
sqledit="select * from jlclgl where ID="&ReplaceBadChar(Trim(request("ID")))

rsedit.open sqledit,connj,1,3
...
dwt.out  "   <td height='22' colspan='2' align='center'><strong>车辆信息管理页</strong></td>"& vbCrLf
dwt.out  "  </tr>  "& vbCrLf
dwt.out  "<tr class='tdbg'>"& vbCrLf
dwt.out  "    <td width='70' height='30'><strong>管理导航：</strong></td>"& vbCrLf
dwt.out  "    <td height='30'><a href='jlclgl.asp'>车辆信息管理首页</a>"
if session("levelclass")=12 then dwt.out  "&nbsp;|&nbsp;<a href='jlclgl.asp?action=add'>添加车辆信息</a>"
dwt.out  "    </td>"& vbCrLf
dwt.out  "  </tr>"& vbCrLf
...
dwt.out  "      <td style=""border-bottom-style: solid;border-width:1px"" width=""40%""><div align=""center""><strong>标题</strong></div></td>"
dwt.out  "      <td width=""6%"" style=""border-bottom-style: solid;border-width:1px""><div align=""center""><strong>选项</strong></div></td>"
dwt.out  "    </tr>"
sqlzbb="SELECT * from jlclgl ORDER BY id DESC"
set rszbb=server.createobject("adodb.recordset")
rszbb.open sqlzbb,connj,1,1
if rszbb.eof and rszbb.bof then 
dwt.out  "<p align='center'>未添加车辆信息</p>" 
else
           ...
           do while not rszbb.eof and rowcount>0
                 ...
                 dwt.out  "      <td style=""border-bottom-style: solid;border-width:1px"" width=""40%""><a href=jlgl_view.asp?id="&rszbb("id")&" target=_blank>"&rszbb("title")&"</a></td>"
                 dwt.out  "      <td width=""6%"" style=""border-bottom-style: solid;border-width:1px""><div align=""center"">"
				 if session("levelclass")=12 then dwt.out  "<a href='jlclgl.asp?action=edit&ID="&rszbb("id")&"'>编辑</a>&nbsp;  <a href='jlclgl.asp?action=del&ID="&rszbb("id")&"' onClick=""return confirm('确定要删除此内容吗？');"">删除</a>"
				 dwt.out  "&nbsp;</div></td>"
                 dwt.out  "    </tr>"
                 RowCount=RowCount-1
          rszbb.movenext
...
sqldel="delete * from jlclgl where id="&id
rsdel.open sqldel,connj,1,3
