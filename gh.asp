...
if classid="" then  classid=2

if indexid<>"" then classname=conndgt.Execute("SELECT class_name FROM dgtzl_index_gh WHERE id="&indexid)(0)
...
sub add()
	...
	dwt.out "<form method='post' action='gh.asp' name='form1'>"
	...
	if classid=2 then 
	  if indexid="" then  indexid=2
	dwt.out "<input name='ssbz' type='hidden' value='"&indexid&"'  />"
	dwt.out "<input name='news_class' type='hidden' value='2'  />"
	classname=conndgt.Execute("SELECT class_name FROM dgtzl_index_gh WHERE id="&indexid)(0)
end if 
	if classid<>2 then 
...
sub saveadd()    
	...
	dwt.savesl conndgt.Execute("SELECT class_name FROM dgtzl_class WHERE id="&request("news_class"))(0) ,"添加",ReplaceBadChar(Trim(Request("news_title")))
      set rsadd=nothing
	  dwt.out "<Script Language=Javascript>location.href='gh.asp?classid="&request("news_class")&"';</Script>"
end sub
...
sub edit()
   ...
   rsedit.open sqledit,conndgt,1,1
	...
	dwt.out "<input name='news_class' type='hidden' value='2'  />"
	classname=conndgt.Execute("SELECT class_name FROM dgtzl_index_gh WHERE id="&rsedit("index"))(0)
end if 
	if classid<>2 then 
	dwt.out "<input name='ssbz' type='hidden' value='0'  />"
	dwt.out "<input name='news_class' type='hidden' value='"&classid&"'  />"
...
dim sqlcsyy,rscsyy,csyy_numb
	...
	if rsnews.eof and rsnews.bof then 
	   ...
	   do while not rsnews.eof and rowcount>0
          ...
          dim classname11
		  if rsnews("news_class")=2 then 
		  'classname11=conndgt.Execute("SELECT class_name FROM dgtzl_index_gh WHERE id="&rsnews("index"))(0)
		  
sqld="SELECT class_name FROM dgtzl_index_gh WHERE id="&rsnews("index")
		  ...
		  if rsnews("news_class")=3 then  classname11="团委"
			...
			if classid=2 then 
			 ...
			 if rsnews("user_id")=0 or session("userid")=79 or session("userid")=1 or rsnews("user_id")=session("userid") then 
				 dwt.out  "<a href='gh.asp?classid="&classid&"&action=edit&ID="&rsnews("id")&"'>编辑</a>&nbsp;"
				 dwt.out "<a href='gh.asp?action=del&ID="&rsnews("id")&"' onClick=""return confirm('确定要删除此内容吗？');"">删除</a>"
			 end if 
			 dwt.out  "&nbsp; </div></td>"
			 dwt.out  "    </tr>"
			 RowCount=RowCount-1
...
sub search()
	...
	Dwt.out "<form method='Get' name='SearchForm' action='gh.asp'>" & vbCrLf
	
	dim urlaction
	if classid<>2 then urlaction="classid="&classid
	if classid=2 then urlaction="indexid="&indexid
	if displaypagelevelh(session("groupid"),1,session("pagelevelid"))  then Dwt.out "<a href=""gh.asp?action=add&"&urlaction&""">添加</a>"
	dwt.out "&nbsp;&nbsp;搜索：" & vbCrLf
