...
sqlpxst="select * from huiyiluoshi where id="&request("id")
...
<div class="boxl">
<div class="t1">
  <div class="dq"><a href="/" class=class>首页</a> &gt; <a href=diaoduhui_d.asp?wangong=0>工作落实</a> </div>
</div>
<p class="br"> </p>
...
<hr>
<div class='bodyt'><br>
  <strong>来源：</strong><%=rspxst("ly")%><br>
  <strong>来源时间：</strong><%=rspxst("huiyi_date")%><br>
...
<div class='bodyt'><strong>任务内容及进度描述如下：</strong><br><%=rspxst("pxst_title")%><%'imgCode(rspxst("pxst_body"))%></div>
<br>
<%set rsqxtb_fk=server.createobject("adodb.recordset")
sqlqxtb_fk="select * from huiyiluoshi_fk where huiyiluoshi_id="&request("id")
