...
sub add()
   '新增用户
   dwt.out"<form method='post' action='bzmanagement.asp' name='form1' onsubmit='javascript:return CheckAdd();'>"
   ...
   dwt.out"<div align='center'><strong>新 增 班 组</strong></div></td>    </tr>"
	 dwt.out"<tr class='tdbg'><td width='12%' align='right' class='tdbg'>"
	 dwt.out"<strong>班 组 名：</strong></td>"
	 dwt.out"<td width='88%' class='tdbg'><input name='bzname' type='text'></td>    </tr>   "
...
sub saveadd()    
      ...
      sqladd="select * from bzname" 
      rsadd.open sqladd,conn,1,3
      rsadd.addnew
      rsadd("bzname")=ReplaceBadChar(Trim(Request("bzname")))
...
sub main()
	...
	dwt.out "        <SPAN class='x-layout-panel-hd-text' style:'top:0px;'>班组管理</span>"& vbCrLf
	dwt.out "     </div>"& vbCrLf
    dwt.out "<div class='x-toolbar'>" & vbCrLf
    dwt.out "<div align=left><a href=bzmanagement.asp?action=add>添加班组</a></div>"
	dwt.out "</div>"
	  sqlbody="SELECT * from bzname"
      ...
      if rsbody.eof and rsbody.bof then 
           dwt.out "<p align=""center"">暂无内容</p>" 
			...
			dwt.out "      <td class='x-td'><DIV class='x-grid-hd-text'>班 组 名</div></td>"
           ...
           do while not rsbody.eof and rowcount>0
			...
			if xh_id mod 2 =1 then 
			  ...
			  dwt.out "<tr class='x-grid-row' onmouseout=""this.style.backgroundColor=''"" onmouseover=""this.style.backgroundColor='#BFDFFF'"">"& vbCrLf
                 ...
                 dwt.out "      <td style=""border-bottom-style: solid;border-width:1px"" ><div align=""center"">"&rsbody("bzname")&"</div></td>"
                 dwt.out "      <td style=""border-bottom-style: solid;border-width:1px"" ><div align=""center"">"&sscjh(rsbody("sscj"))&"</div></td>"
                  dwt.out "      <td  style=""border-bottom-style: solid;border-width:1px""><div align=""center""><a href='bzmanagement.asp?action=edit&ID="&rsbody("id")&"'>编辑</a>&nbsp;"
				 dwt.out "  <a href='bzmanagement.asp?action=del&ID="&rsbody("id")&"' onClick=""return confirm('确定要删除此班组吗？删除后其他相关的此班组内容将不会显示');"">删除</a></div></td>"
                 dwt.out "    </tr>"
                 RowCount=RowCount-1
          rsbody.movenext
          loop
