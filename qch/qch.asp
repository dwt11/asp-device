...
if request("wupin")<>"" then sql=sql&" and  品种 like '%" &request("wupin")& "%' "	
	...
	response.Write "        <SPAN class='x-layOut-panel-hd-text' style:'top:0px;'>汽车衡报表 "&dateinput&"</span>"& vbCrLf
	...
	response.Write "		 <form method='post'  action='qch.asp'  name='form' >"
	
	'response.Write dateinput
	response.Write "<a href='qch.asp?year="&year(dateinput-2)&"&month="&month(dateinput-2)&"&day="&day(dateinput-2)&"'>"&year(dateinput-2)&"年"&month(dateinput-2)&"月"&day(dateinput-2)&"日</a>&nbsp;&nbsp;&nbsp;&nbsp;"
	response.Write "<a href='qch.asp?year="&year(dateinput-1)&"&month="&month(dateinput-1)&"&day="&day(dateinput-1)&"'>"&year(dateinput-1)&"年"&month(dateinput-1)&"月"&day(dateinput-1)&"日</a>&nbsp;&nbsp;&nbsp;&nbsp;"
	...
	if now()-dateinput>1 then 	response.Write "<a href='qch.asp?year="&year(dateinput+1)&"&month="&month(dateinput+1)&"&day="&day(dateinput+1)&"'>"&year(dateinput-1)&"年"&month(dateinput+1)&"月"&day(dateinput+1)&"日</a>&nbsp;&nbsp;&nbsp;&nbsp;"
	if now()-dateinput>2 then 	response.Write "<a href='qch.asp?year="&year(dateinput+2)&"&month="&month(dateinput+2)&"&day="&day(dateinput+2)&"'>"&year(dateinput+2)&"年"&month(dateinput+2)&"月"&day(dateinput+2)&"日</a>&nbsp;&nbsp;&nbsp;&nbsp;"
...
response.Write "<td class='x-td'><Div class='x-grid-hd-text'>原发净重</Div></td>" & vbCrLf	
		response.Write "<td class='x-td'><Div class='x-grid-hd-text'>净差</Div></td>" & vbCrLf	

response.Write "<td class='x-td'><Div class='x-grid-hd-text'>原发皮重</Div></td>" & vbCrLf	
response.Write "<td class='x-td'><Div class='x-grid-hd-text'>皮差</Div></td>" & vbCrLf	
...
'			end if	
		...
		response.Write "     <td  CLASS='X-TD'><Div align=""center"">"&rs("载重")&"</Div></td>"& vbCrLf	
response.Write "     <td  CLASS='X-TD'><Div align=""center"">"&rs("盈亏")&"</Div></td>"& vbCrLf	
response.Write "     <td  CLASS='X-TD'><Div align=""center"">"&rs("标志1")&"</Div></td>"& vbCrLf	
response.Write "     <td  CLASS='X-TD'><Div align=""center"">"&rs("标志2")&"</Div></td>"& vbCrLf	
