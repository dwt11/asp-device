...
dwt.out "<html>"& vbCrLf
dwt.out "<head>" & vbCrLf
dwt.out "<title>信息管理系统-年度检修计划材料汇总</title>"& vbCrLf
...
dim rscj,sqlcj
	...
	dwt.out"		<H3 style='MARGIN-BOTTOM: 5px'>添加年度检修材料</H3>"& vbCrLf
	dwt.out"		<DIV id=form-ct>"& vbCrLf
	dwt.out "<form method='post' class='x-form' action='jxcl.asp' name='form1' onsubmit='javascript:return checkadd();'>"
	...
	dwt.out"			  <DIV class='x-form-item'>"& vbCrLf
	dwt.out"				<LABEL style='WIDTH: 75px' >型号:</LABEL>"& vbCrLf
	dwt.out"				<DIV class='x-form-element' style='PADDING-LEFT: 80px'>"& vbCrLf
	...
	dwt.out"			  <DIV class='x-form-item'>"& vbCrLf
	dwt.out"				<LABEL style='WIDTH: 75px'>材质:</LABEL>"& vbCrLf
	dwt.out"				<DIV class='x-form-element' style='PADDING-LEFT: 80px'>"& vbCrLf
	...
	dwt.out"			  <DIV class='x-form-item'>"& vbCrLf
	dwt.out"				<LABEL style='WIDTH: 75px'>单位:</LABEL>"& vbCrLf
	dwt.out"				<DIV class='x-form-element' style='PADDING-LEFT: 80px'>"& vbCrLf
	...
	dwt.out"			  <DIV class='x-form-item'>"& vbCrLf
	dwt.out"				<LABEL style='WIDTH: 75px'>数量:</LABEL>"& vbCrLf
	dwt.out"				<DIV class='x-form-element' style='PADDING-LEFT: 80px'>"& vbCrLf
	...
	dwt.out"			  <DIV class='x-form-item'>"& vbCrLf
	dwt.out"				<LABEL style='WIDTH: 75px'>定货日期:</LABEL>"& vbCrLf
	dwt.out"				<DIV class='x-form-element' style='PADDING-LEFT: 80px'>"& vbCrLf
	...
	dwt.out"			  <DIV class=x-form-clear-left></DIV>"& vbCrLf
	dwt.out"			  <DIV class='x-form-item'>"& vbCrLf
	dwt.out"				<LABEL style='WIDTH: 75px'>计划到货日期:</LABEL>"& vbCrLf
	dwt.out"				<DIV class='x-form-element' style='PADDING-LEFT: 80px'>"& vbCrLf
	...
	dwt.out"			  <DIV class='x-form-item'>"& vbCrLf
	dwt.out"				<LABEL style='WIDTH: 75px'>实际到货日期:</LABEL>"& vbCrLf
	dwt.out"				<DIV class='x-form-element' style='PADDING-LEFT: 80px'>"& vbCrLf
...
sub saveadd()    
	...
	rsadd.close
	set rsadd=nothing
	dwt.out"<Script Language=Javascript>location.href='jxcl.asp';</Script>"
end sub


sub saveedit()    
	set rsedit=server.createobject("adodb.recordset")
	sqledit="select * from ndjx_cl where id="&ReplaceBadChar(Trim(request("ID")))
...
sub del()
	...
	sqldel="delete * from ndjx_cl where id="&qxdjid
...
sub edit()
	...
	set rsedit=server.createobject("adodb.recordset")
	sqledit="select * from ndjx_cl where id="&id
	...
	dwt.out"		<DIV id=form-ct>"& vbCrLf
    dwt.out"<form method='post' action='jxcl.asp' name='form1' onsubmit='javascript:return checkadd();'>"
	dwt.out"			<DIV class='x-form-ct'>"& vbCrLf
...
sub main()
	...
	sqlqxdj="SELECT * from ndjx_cl  where 1=1"
	...
	dwt.out "        <SPAN class='x-layout-panel-hd-text' style:'top:0px;'>年度检修材料汇总"&title&"</span>"& vbCrLf
	dwt.out "     </div>"& vbCrLf
    ...
    dwt.out "<a href=jxcl.asp?action=edit&id="&rsqxdj("id")&">编辑</a>&nbsp;"
    dwt.out "<a href=jxcl.asp?action=del&id="&rsqxdj("id")&" onClick=""return confirm('确定要删除此记录吗？');"">删除</a>"
 end if 
 dwt.out "&nbsp;"
...
sub search()
	...
	Dwt.out "<form method='Get' name='SearchForm' action='jxcl.asp'>" & vbCrLf
	
	if displaypagelevelh(session("groupid"),1,session("pagelevelid")) then Dwt.out "<a href=""jxcl.asp?action=add"">添加记录</a>&nbsp;&nbsp;"
	...
	sqlgh="SELECT distinct sscj from ndjx_cl"
    ...
    do while not rsgh.eof
		cjid=cint(rsgh("sscj"))
		sql="SELECT count(jx_id) FROM ndjx_cl WHERE sscj="&cjid
		if request("jx_nd")<>"" then sql=sql&" and jx_nd="&request("jx_nd")
		jx_numb=connnd.Execute(sql)(0)
        
		if jx_numb<>0 then 
			'i=i+1
			Dwt.out"<option  value='jxcl.asp?sscj="&cjid
	...
	sqlgh="SELECT distinct jx_nd from ndjx_cl"
	if request("sscj")<>"" then sqlgh=sqlgh&" where sscj="&request("sscj")
	set rsgh=server.createobject("adodb.recordset")
    rsgh.open sqlgh,connndjx,1,1
    do while not rsgh.eof
		jx_nd=cint(rsgh("jx_nd"))
		sql="SELECT count(id) FROM ndjx_cl WHERE jx_nd="&jx_nd
		if request("sscj")<>"" then sql=sql&" and sscj="&request("sscj")
		jx_numb=connnd.Execute(sql)(0)
        
		if jx_numb<>0 then 
			i=i+1
			Dwt.out"<option  value='jxcl.asp?jx_nd="&jx_nd&"'"
