...
AutoAssist.prototype = {
	...
	show: function() {
		...
		this.status = "show";
		Transition(this.floatDiv,'x','block');			// 显示结果
		Element.show(this.floatDiv);
	},

	hide: function() {
		this.status = "none";
		Transition(this.floatDiv,'x','none');			//隐藏查询结果
		Element.hide(this.floatDiv);
	}
}
/*过渡效果*/
function Transition(DivID,Cur,DoType) 
{ 
	(Cur!='x')?DivID.filters.revealTrans.Transition=Cur:"";
	(DoType == '')?DoType = "none":"";
	DivID.filters.blendTrans.apply();
	DivID.style.display = DoType; 
