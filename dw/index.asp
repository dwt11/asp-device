...
<title><%=classname%>党建-信息管理系统</title>
...
<DIV class="box960">
  <div class="boxl">
    <div class="t1">
      <div class="dq">当前位置：<a href="/">首页</a> > <a href="/dw">党建</a>  <%=gettclassname(classid)%> </div>
    </div>
    <%dim ii
		if classid="" then sqltree="SELECT * from dgtzl_index where index=0 order by orderby asc"
		if classid<>"" then sqltree="SELECT * from dgtzl_index where index="&classid&"  and id<>18 order by orderby asc"
		...
		do while not rstree.eof
			...
			'dwt.out "<br>"
				...
				sqltree3="SELECT top 7 * from dgtzl_body where index in("&classid1&")  and news_class=1  order by id desc"& vbCrLf
