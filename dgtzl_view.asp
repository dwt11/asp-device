...
<table width="760" border=0 align="center" cellpadding=0 cellspacing=0>
  <tr>
    <td class=main_title_1i>当前位置：<a href="/" class=class>信息管理系统首页</a>&gt;&gt;&gt; <a href="dgtzl.asp?CLASSid=<%=rsnews("news_class")%>"><%=newsclassh(rsnews("news_class"))%></a> &gt;&gt;&gt; <%=rsnews("news_title")%></td>
  </tr>
  <tr>
      ...
      <table width="80%"  border="0" align="center" cellpadding="0" cellspacing="0">
        <tr>
          <%if rsnews("user_id")=0 then 
				  ...
				  %>
          <td align="center">发布时间：<font color="#990000"><%=rsnews("news_date")%></font><%if rsnews("news_class")<>21 then%>　 发布：<font color="#990000"><%=news_zz%> </font><%end if%></td>
        </tr>
        <%
...
'    if isre then 
'		dwt.out "<br/><br/><br/>当前1楼，<a href=news.asp?action=addpost&news_id="&rsnews("id")&">回复</a><br/> <hr width=100% size=1><br/><br/><br/>"
'		post=1
...
'			post=post+1
'			dwt.out rspost("body")
'			dwt.out "<br/><br/><br/>当前"&post&"楼，"&rspost("date")&"<br/><hr width=100% size=1><br/><br/><br/>"
'		  rspost.movenext
'		  loop
'        end if
'	end if  
...
'                    response.write usergroupid&"ooooooooooooooooo"
				...
				if  rsnews("viewd")<>"" then 
					...
					if rsnews("viewdgroup")=2 or rsnews("viewdgroup")=3  or isnull(rsnews("viewdgroup")) or rsnews("viewdgroup")=0  then  sqlcj1="SELECT * from levelname "  '111220修改添加 OR ISNULL 当VIEWDGROUP为空时也输出人名
