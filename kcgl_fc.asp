...
if Request("class")="" and request("sscj")="" then pagename="出库台账"
...
do while not rsclass.eof
   ...
   rs.open sql,connkc,1,1
   do while not rs.eof
	  response.write "<a href=kcgl_fc.asp?class="&rs("id")&">"&rs("name")&"</a>&nbsp;|&nbsp;"& vbCrLf
   rs.movenext
   loop
   rs.close
   set rs=nothing
...
if Request("action")="delfcinfo" then call delfcinfo  '删除出库信息
...
if request("class")<>"" then 
   url="kcgl_fc.asp?class="&request("class")
   sqlbody="SELECT * from fc where class="&request("class")&" order by id DESC"
end if 

if request("sscj")<>"" then 
   url="kcgl_fc.asp?sscj="&request("sscj")
   sqlbody="SELECT * from fc where sscj="&request("sscj")&" order by id DESC"
end if 
  'sqlbody="SELECT * from fc order by id DESC"
  ...
  do while not rsbody.eof and rowcount>0
		...
		if rsbody("qxtxt")=1000 then 
		   ...
		   response.write sscjh(rsbody("qxtxt"))&"&nbsp;</div></td>"
        end if 
...
dim rsdel,sqldel
  ...
  sqldel="delete * from fc where id="&request("id")
...
response.write "   <td>  <strong>出库信息搜索：</strong>" & vbCrLf
...
response.write "  <input type='hidden' name='action' value='fc'>" & vbCrLf
...
sqlcj="SELECT * from levelname where levelclass=1 "& vbCrLf
    ...
    do while not rscj.eof
       	response.write"<option value='kcgl_fc.asp?sscj="&rscj("levelid")&"'>"&rscj("levelname")&"</option>"& vbCrLf
